...
В этом примере показано, как можно использовать класс `PersistentList` для создания двусвязного списка, добавления элементов, клонирования, получения старых версий и удаления элементов.
...
p_list.add(2)  # Добавляем элемент 2 -> версия 1
p_list.add(3)  # Добавляем элемент 3 -> версия 2

# Выводим текущее состояние списка
print("Текущий список:", p_list._get_current_state())  # [1, 2, 3]
```
...
print("Элемент по индексу 1:", p_list.get(1))  # 2
```
...
```python
# Создаем новую версию с добавлением элемента -> версия 3
p_list.add(4)
print("Состояние списка после добавления 4:", p_list._get_current_state())  # [1, 2, 3, 4]
```

```console
Состояние списка после добавления элемента: [1, 2, 3, 4]
```

```python
# Клонируем текущий список
cloned_list = p_list.clone()
print("Клонированный список:", cloned_list._get_current_state())  # [1, 2, 3, 4]
```

```console
Клонированный список: [1, 2, 3, 4]
```

```python
# Удаляем элемент по индексу 2 (элемент 3) -> версия 4
p_list.remove(2)
print("Состояние списка после удаления элемента с индексом 2:", p_list._get_current_state())  # [1, 2, 4]
```

```console
Состояние списка после удаления элемента с индексом 2: [1, 2, 4]
```

```python
# Пробуем получить элемент из старой версии (когда добавили 3 элемента)
print("Состояние списка на версии 2:", p_list.get_version(2))  # [1, 2, 3]
```

```console
Состояние списка на версии 2: [1, 2, 3]
```

```python
# Пробуем получить элемент из старой версии (когда добавили четвертый элемент)
print("Состояние списка на версии 3:", p_list.get_version(3))  # [1, 2, 3, 4]
```

```console
Состояние списка на версии 3: [1, 2, 3, 4]
```

```python
# Обновляем текущую версию списка на версию 3 (когда добавили четвертый элемент)
p_list.update_version(3)
print("Состояние списка после обновления до версии 3:", p_list._get_current_state())  # [1, 2, 3, 4]
```

```console
Состояние списка после обновления до версии 3: [1, 2, 3, 4]
```

```python
# Очищаем список
p_list.clear()
print("Состояние списка после очистки:", p_list._get_current_state())  # []
```

```console
Состояние списка после очистки: []
```
...
try:
    ...
    print("Ошибка:", e)  # Ошибка: Неверный индекс
```
